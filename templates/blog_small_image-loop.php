...
/*
 * Added by FreshWeb Studio, 2017-01-23
 * Reformat date string displayed on blog post subtitles.
 */
$blog_the_time = get_the_time( 'F j, Y' );
?>
<?php
	switch ($_post_format) {
		case "video":
...
?>
		<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<div class="post_content_holder">
				...
				<div class="post_text">
					<div class="post_text_inner">
					    <!-- FreshWeb Studio, 2017-01-23
					         1. Removed the date string from title.
					         2. Add date string to subtitle.
					    -->
						<h2 itemprop="name" class="entry_title"><meta itemprop="interactionCount" content="UserComments: <?php echo get_comments_number(qode_get_page_id()); ?>"/></span> <a itemprop="url" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php the_title(); ?></a></h2>
						<div class="post_info">
							<span class="time"><?php _e('Posted','qode'); ?> <?php echo $blog_the_time; ?></span>
							<?php _e('in','qode'); ?> <?php the_category(', '); ?>
                            <?php if($blog_hide_author == "no") { ?>
                                <span class="post_author">
                                    <?php _e('by','qode'); ?>
